...
### Persistent PPTX Status Reports with SPE Storage
**Status:** Design Required
**Effort:** Medium (4-6 weeks)

**Overview:** Currently AI status reports are generated on-demand and not persisted. This feature adds a `status_reports` document type stored in SharePoint Embedded (preferred) or available for local download, with full lifecycle management in the Projects UI.

**Data Model:**
- [ ] New `status_reports` table: id, projectId, tenantId, asOfDate, generatedBy, createdAt, speFileId (nullable), speContainerId (nullable), title, status (draft/final)
- [ ] New SPE document type registration for status reports (alongside invoices, SOWs, expenses, deliverables)

**Generation Flow:**
- [ ] On "Generate Status Report," persist the report record to the database first, then save the PPTX to SPE, then trigger the browser download
- [ ] Store the SPE file reference (fileId + containerId) on the status report record so it can be retrieved later
- [ ] Fallback: if SPE storage fails, still allow local download but flag the report as "not stored"

**Projects UI — Status Reports Tab:**
- [ ] Add a "Status Reports" tab to the Project detail interface (alongside Deliverables, RAIDD, Time, Expenses, etc.)
- [ ] List all saved status reports for the project with columns: title, as-of date, generated by, created date, status (draft/final)
- [ ] Actions per report: download PPTX, view in SPE (open in browser), delete
- [ ] Bulk delete option for cleaning up older drafts
- [ ] Confirmation dialog before deletion with count of selected reports

**MCP Integration:**
- [ ] Wire up `/mcp/projects/:projectId/status-reports` endpoint (currently returns empty stub) to query persisted status reports
- [ ] Ensure Copilot/AI assistants can discover and reference saved status reports

**Edge Cases:**
- [ ] Handle regeneration: generating a new report for the same as-of date should create a new version, not overwrite
- [ ] Retention: consider auto-flagging reports older than N months as archivable
- [ ] RBAC: PMs and above can generate/delete; team members can view/download
