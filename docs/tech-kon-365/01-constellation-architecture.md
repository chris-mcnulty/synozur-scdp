...
- **Cryptographic per-tenant separation via envelope encryption (the gold-standard pattern, implemented in Zenith).** Zenith's **"Data Masking"** feature gives each tenant **its own AES-256-GCM data-encryption key**. The mechanism is textbook envelope encryption: a single master key (held in the environment, stretched with PBKDF2) *wraps* a randomly generated per-tenant key, and that wrapped key is stored one-row-per-tenant in a dedicated `tenant_encryption_keys` table. Sensitive fields — display names, site/team names, user principal names, file names and URLs, even Copilot prompt text — are then encrypted **with that specific tenant's key** and written into the shared database as ciphertext (`MASKED:` + IV + auth tag + data). Anyone querying the database directly sees only encrypted values for that tenant; the application decrypts to real values for authorized users. That turns "we filter by tenant" into "we *cannot* return another tenant's sensitive data even by mistake" — isolation enforced by cryptography, not just by correct code.
- **And it's a control the admin owns, per tenant.** Data Masking is an **admin-controlled, per-tenant opt-in toggle** (Tenant Admin / Platform Owner), gated by plan and audited. Turning it on generates the tenant's key and bulk-encrypts existing rows; turning it off bulk-decrypts. A read chokepoint decrypts on the way out and scrubs any value it can't decrypt, so a missing key fails closed rather than leaking ciphertext. *This* is what "per-tenant control of encryption" means concretely.
...
> **Quality-pattern note for the deck:** frame this as a spectrum the audience can place their own systems on — *logical isolation (filter by tenant) → per-tenant identity/content boundary → per-tenant data-encryption keys*. Zenith sits at the far end (its own key per tenant, admin-toggled); Constellation shows the content-layer boundary in production (per-customer Azure tenant + SharePoint Embedded container) plus a versioned encryption format on the path to per-tenant data keys. The teaching point is the pattern, not any one product.
>
> If you expect a sharp technical audience, know the honest edges of Zenith's implementation so you can answer well: it's **envelope encryption with an operator-held master key** (not customer-managed keys / BYOK, and not backed by a KMS/Key Vault today), it encrypts **specific sensitive fields within a shared database** (not a separate database or schema per tenant), and there's **no key-rotation endpoint yet** — rotating the master secret would strand the wrapped per-tenant keys. None of that weakens the core pattern; it's the natural "what's next" (KMS-backed master key, BYOK, rotation) that makes a great forward-looking slide.
