...
- **Salaried Resource Handling**: Users can be marked as "salaried" individually (`isSalaried`) or via their role's "always salaried" setting (`isAlwaysSalaried`). **Precedence**: Individual employee setting always takes precedence over role configuration. Role's salaried flag is a fallback used in estimates when specific staffing isn't decided. Salaried resources' time is not counted as direct project cost in margin calculations for estimates, time entries, and financial reports. This allows principals and other salaried team members to log time without impacting project profitability metrics. Default salaried roles: Principal, Senior Architect.
