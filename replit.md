...
- **Per-Tenant Azure AD Isolation**: Each tenant's `azureTenantId` is auto-populated from the sign-in token (`tid` claim) on first SSO login. Container creation and Graph API calls use the tenant's own Azure AD tenant ID via `getClientCredentialsMsalForTenant()`, ensuring customer tenants never access Synozur's containers. MSAL instances are cached per Azure AD tenant ID. The sign-in flow uses `common` authority (multi-tenant); client credentials flow uses per-tenant authority. Key files: `server/auth/entra-config.ts` (`getClientCredentialsMsalForTenant`), `server/services/container-creator.ts`, `server/services/graph-client.ts`.
