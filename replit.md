...
- **Financial Reporting**: Comprehensive reports showing revenue, cost, profit, and margins by client/project, with KPI summaries and health scoring. Invoice Report page with as-of-date filtering, tenant-timezone-aware date handling, and batch type filters.
  - **Backlog (Low Priority — Post-Project-Close)**: Invoice-level profitability analysis. Calculate profit per invoice by factoring in native tax, subcontractor charges, and cost rates. May not be fully accurate until after a project is closed, as contractor invoices can trail by 30 days. Requires linking ongoing projects to their estimates to determine if a referral associate commission (typically 5% of net profit) needs to be carried over. Considerations: (1) subcontractor invoice lag, (2) estimate-to-project linkage for commission detection, (3) net profit = invoice revenue − labor cost − subcontractor charges − tax − commission.
