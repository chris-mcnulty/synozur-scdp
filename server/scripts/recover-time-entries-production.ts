...
import { timeEntries, users, projects } from '@shared/schema';
...
async function recoverTimeEntries(filePath: string, options: {
  isDryRun: boolean;
  checkDependencies: boolean;
  ignoreForeignKeys: boolean;
}) {
  console.log('=== Time Entries Recovery Script (Production Version) ===');
  console.log(`Mode: ${options.isDryRun ? 'DRY RUN (no changes will be made)' : 'LIVE (data will be imported)'}`);
  console.log(`Options: `);
  console.log(`  - Check Dependencies: ${options.checkDependencies}`);
  console.log(`  - Ignore Foreign Keys: ${options.ignoreForeignKeys}`);
  ...
  // Extract unique user IDs and project IDs
  const uniqueUserIds = [...new Set(data.map(e => e.person_id))];
  const uniqueProjectIds = [...new Set(data.map(e => e.project_id))];

  console.log(`Unique users in backup: ${uniqueUserIds.length}`);
  console.log(`Unique projects in backup: ${uniqueProjectIds.length}`);
  console.log('');
  ...
  if (options.checkDependencies) {
    ...
    const missingProjectIds = uniqueProjectIds.filter(id => !existingProjectIds.has(id));
    
    if (missingUserIds.length > 0) {
      console.log(`⚠️  Missing ${missingUserIds.length} users:`);
      missingUserIds.forEach(id => console.log(`    - ${id}`));
      console.log('');
    }
    
    if (missingProjectIds.length > 0) {
      console.log(`⚠️  Missing ${missingProjectIds.length} projects:`);
      missingProjectIds.forEach(id => console.log(`    - ${id}`));
      console.log('');
    }
    
    if (!options.ignoreForeignKeys && (missingUserIds.length > 0 || missingProjectIds.length > 0)) {
      console.log('❌ Cannot proceed: Missing dependencies detected.');
      console.log('   Options:');
      console.log('   1. Import missing users and projects first');
      console.log('   2. Run with --ignore-foreign-keys flag (for production recovery)');
      return;
    }
  }
  ...
  if (!options.isDryRun) {
    ...
    const entriesToImport = transformedEntries.filter(e => !existingIds.has(e.id));
    const duplicateEntries = transformedEntries.filter(e => existingIds.has(e.id));

    if (duplicateEntries.length > 0) {
      console.log(`⚠️  Found ${duplicateEntries.length} entries that already exist in database (will skip)`);
      console.log('');
    }

    if (entriesToImport.length === 0) {
      console.log('✓ All entries already exist in database. No import needed.');
      return;
    }

    console.log(`Importing ${entriesToImport.length} new entries...`);
    console.log('');
    ...
    // Optionally disable foreign key checks for production recovery
    if (options.ignoreForeignKeys) {
      console.log('⚠️  DISABLING FOREIGN KEY CHECKS (Production Recovery Mode)');
      await db.execute(sql`SET session_replication_role = 'replica'`);
    }

    for (let i = 0; i < entriesToImport.length; i += batchSize) {
      const batch = entriesToImport.slice(i, i + batchSize);
      console.log(`Processing batch ${Math.floor(i / batchSize) + 1} (entries ${i + 1}-${Math.min(i + batchSize, entriesToImport.length)})...`);
    ...
    console.log(`⚠️  Skipped (already exist): ${duplicateEntries.length} entries`);
    if (errorCount > 0) {
      console.log(`✗ Failed to import: ${errorCount} entries`);
      console.log('');
      console.log('Failed entries (first 10):');
    ...
    console.log('  --live                  Actually import the data');
    console.log('  --check-deps            Check for missing users/projects');
    console.log('  --ignore-foreign-keys   Bypass foreign key constraints (PRODUCTION ONLY)');
  }
}
...
async function main() {
  ...
  const checkDependencies = args.includes('--check-deps');
  const ignoreForeignKeys = args.includes('--ignore-foreign-keys');
  const filePath = args.find(arg => !arg.startsWith('--')) || 'attached_assets/time_entries (1)_1759678506686.xlsx';

  if (ignoreForeignKeys && isDryRun) {
    console.error('⚠️  Warning: --ignore-foreign-keys flag has no effect in dry-run mode');
    console.log('');
  }

  try {
    await recoverTimeEntries(filePath, { isDryRun, checkDependencies, ignoreForeignKeys });
    process.exit(0);
  } catch (error) {
    console.error('');
