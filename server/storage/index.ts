...
export interface IStorage {
  ...
  getTimeEntriesPaginated(filters: { personId?: string; projectId?: string; clientId?: string; startDate?: string; endDate?: string; tenantId?: string; billable?: boolean; search?: string; allocationId?: string; epicId?: string; workstreamId?: string; projectStageId?: string; commercialTreatment?: string; submissionStatus?: string; limit: number; offset: number }): Promise<{ items: any[]; total: number; hasMore: boolean; totals?: { hours: number; billableHours: number; nonBillableHours: number } }>;
  ...
  raiddEntryHasResolutionBacking(entryId: string): Promise<boolean>;
  resolveRaiddEntry(entryId: string, options: { userId: string; resolveStatus: string; resolutionNotes?: string; path: 'decision' | 'action'; decision?: { title: string; description?: string; decisionDate?: string | null; ownerId?: string | null; stakeholderIds?: string[] | null }; actionItemId?: string }): Promise<{ entry: RaiddEntry; decision?: RaiddEntry }>;
  ...
  getMyRaiddEntries(userId: string, tenantId: string, filters?: { type?: string; status?: string; priority?: string; projectId?: string }): Promise<(RaiddEntry & { ownerName?: string; assigneeName?: string; createdByName?: string; projectName?: string; clientName?: string; myRole?: string })[]>;
  ...
  createGalaxyApiKey(data: { tenantId: string; appId: string; clientId: string; name: string; scopes: string[]; expiresAt?: Date | null; createdBy?: string }): Promise<{ raw: string; record: any }>;
  listGalaxyApiKeys(appId: string, tenantId: string): Promise<any[]>;
  lookupGalaxyApiKeyByHash(hash: string): Promise<{ key: any; app: any } | null>;
  revokeGalaxyApiKeyById(keyId: string, tenantId: string): Promise<void>;
  touchGalaxyApiKeyUsed(keyId: string): Promise<void>;
