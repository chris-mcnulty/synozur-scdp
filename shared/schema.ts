...
// ============================================================================
// HELP DESK & FEEDBACK SYSTEM (Multi-App Support)
// ============================================================================

// Ticket status enum
export const helpdeskTicketStatusEnum = z.enum(['open', 'in_progress', 'waiting_on_user', 'resolved', 'closed']);
export type HelpdeskTicketStatus = z.infer<typeof helpdeskTicketStatusEnum>;

// Ticket priority enum
export const helpdeskTicketPriorityEnum = z.enum(['low', 'medium', 'high', 'critical']);
export type HelpdeskTicketPriority = z.infer<typeof helpdeskTicketPriorityEnum>;

// Ticket category enum
export const helpdeskTicketCategoryEnum = z.enum(['bug', 'question', 'access_request', 'data_issue', 'performance', 'other']);
export type HelpdeskTicketCategory = z.infer<typeof helpdeskTicketCategoryEnum>;

// Feedback status enum
export const helpdeskFeedbackStatusEnum = z.enum(['submitted', 'acknowledged', 'under_review', 'backlog', 'roadmap', 'declined', 'implemented']);
export type HelpdeskFeedbackStatus = z.infer<typeof helpdeskFeedbackStatusEnum>;

// Feedback type enum
export const helpdeskFeedbackTypeEnum = z.enum(['general', 'feature_request', 'wishlist', 'improvement', 'usability']);
export type HelpdeskFeedbackType = z.infer<typeof helpdeskFeedbackTypeEnum>;

// Registered apps that can submit to help desk
export const helpdeskApps = pgTable("helpdesk_apps", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  appCode: varchar("app_code", { length: 50 }).notNull().unique(), // 'constellation', 'orion', 'nebula', 'vega'
...
export const insertHelpdeskAppSchema = createInsertSchema(helpdeskApps).omit({ id: true, createdAt: true });
export type InsertHelpdeskApp = z.infer<typeof insertHelpdeskAppSchema>;
export type HelpdeskApp = typeof helpdeskApps.$inferSelect;

// Help desk tickets
export const helpdeskTickets = pgTable("helpdesk_tickets", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").references(() => tenants.id).notNull(),
  appId: varchar("app_id").references(() => helpdeskApps.id).notNull(),
  
  // Ticket info
  ticketNumber: varchar("ticket_number", { length: 20 }).notNull().unique(), // HD-2026-001
  subject: text("subject").notNull(),
  description: text("description").notNull(),
  category: text("category").notNull().default('other'), // bug, question, access_request, etc.
  ...
  // Metadata
  pageUrl: text("page_url"), // Where the ticket was submitted from
  browserInfo: text("browser_info"),
  additionalContext: jsonb("additional_context"), // Any extra data from the app
  
  createdAt: timestamp("created_at").notNull().default(sql`now()`),
  updatedAt: timestamp("updated_at").notNull().default(sql`now()`),
}, (table) => ({
  tenantIdIdx: index("helpdesk_tickets_tenant_id_idx").on(table.tenantId),
  appIdIdx: index("helpdesk_tickets_app_id_idx").on(table.appId),
  statusIdx: index("helpdesk_tickets_status_idx").on(table.status),
  submittedByIdx: index("helpdesk_tickets_submitted_by_idx").on(table.submittedByUserId),
  assignedToIdx: index("helpdesk_tickets_assigned_to_idx").on(table.assignedToUserId),
}));
...
export const insertHelpdeskTicketSchema = createInsertSchema(helpdeskTickets).omit({ 
  ...
  createdAt: true, 
  updatedAt: true,
  ticketNumber: true, // Generated by system
});
...
export const helpdeskFeedback = pgTable("helpdesk_feedback", {
  ...
  // Feedback info
  feedbackNumber: varchar("feedback_number", { length: 20 }).notNull().unique(), // FB-2026-001
  title: text("title").notNull(),
  description: text("description").notNull(),
  feedbackType: text("feedback_type").notNull().default('general'), // general, feature_request, wishlist, improvement, usability
  status: text("status").notNull().default('submitted'), // submitted, acknowledged, under_review, backlog, roadmap, declined, implemented
  ...
  // Voting/Priority (for future community voting)
  upvotes: integer("upvotes").default(0),
  impactScore: integer("impact_score"), // Admin-assigned impact score
  
  // Acknowledgement and workflow
  acknowledgedAt: timestamp("acknowledged_at"),
  acknowledgedByUserId: varchar("acknowledged_by_user_id").references(() => users.id),
  adminNotes: text("admin_notes"),
  declineReason: text("decline_reason"),
  
  // Roadmap info (when promoted)
  targetRelease: text("target_release"), // e.g., "Q2 2026", "v2.5"
  ...
  tenantIdIdx: index("helpdesk_feedback_tenant_id_idx").on(table.tenantId),
  appIdIdx: index("helpdesk_feedback_app_id_idx").on(table.appId),
  statusIdx: index("helpdesk_feedback_status_idx").on(table.status),
  feedbackTypeIdx: index("helpdesk_feedback_type_idx").on(table.feedbackType),
  submittedByIdx: index("helpdesk_feedback_submitted_by_idx").on(table.submittedByUserId),
}));

export const insertHelpdeskFeedbackSchema = createInsertSchema(helpdeskFeedback).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true,
  feedbackNumber: true, // Generated by system
  upvotes: true,
  acknowledgedAt: true,
  acknowledgedByUserId: true,
  implementedAt: true,
});
export type InsertHelpdeskFeedback = z.infer<typeof insertHelpdeskFeedbackSchema>;
export type HelpdeskFeedback = typeof helpdeskFeedback.$inferSelect;

// Attachments for tickets and feedback (screenshots, files)
export const helpdeskAttachments = pgTable("helpdesk_attachments", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").references(() => tenants.id).notNull(),
  
  // Parent reference (either ticket or feedback)
  ticketId: varchar("ticket_id").references(() => helpdeskTickets.id),
  feedbackId: varchar("feedback_id").references(() => helpdeskFeedback.id),
  
  // File info
  fileName: text("file_name").notNull(),
  fileType: text("file_type").notNull(), // image/png, image/jpeg, application/pdf
  fileSize: integer("file_size").notNull(), // bytes
  storageKey: text("storage_key").notNull(), // Object storage key
  thumbnailKey: text("thumbnail_key"), // For images, a thumbnail version
  ...
  tenantIdIdx: index("helpdesk_attachments_tenant_id_idx").on(table.tenantId),
  ticketIdIdx: index("helpdesk_attachments_ticket_id_idx").on(table.ticketId),
  feedbackIdIdx: index("helpdesk_attachments_feedback_id_idx").on(table.feedbackId),
}));
...
// Planner sync for tickets (two-way sync with Microsoft Planner)
export const helpdeskPlannerSync = pgTable("helpdesk_planner_sync", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").references(() => tenants.id).notNull(),
  ticketId: varchar("ticket_id").references(() => helpdeskTickets.id).notNull().unique(),
  
  // Planner info
  planId: text("plan_id").notNull(), // Microsoft Planner plan ID
  bucketId: text("bucket_id").notNull(), // "Vega" bucket ID
  plannerTaskId: text("planner_task_id").notNull(), // Task ID in Planner
  
  // Sync state
  lastSyncedAt: timestamp("last_synced_at").notNull().default(sql`now()`),
  lastSyncDirection: text("last_sync_direction").notNull(), // 'to_planner', 'from_planner'
  syncStatus: text("sync_status").notNull().default('synced'), // 'synced', 'pending', 'error'
  syncError: text("sync_error"),
  
  // ETag for conflict detection
  plannerEtag: text("planner_etag"),
  localVersion: integer("local_version").default(1),
  
  createdAt: timestamp("created_at").notNull().default(sql`now()`),
  updatedAt: timestamp("updated_at").notNull().default(sql`now()`),
}, (table) => ({
  tenantIdIdx: index("helpdesk_planner_sync_tenant_id_idx").on(table.tenantId),
  plannerTaskIdIdx: index("helpdesk_planner_sync_task_id_idx").on(table.plannerTaskId),
  syncStatusIdx: index("helpdesk_planner_sync_status_idx").on(table.syncStatus),
}));

export const insertHelpdeskPlannerSyncSchema = createInsertSchema(helpdeskPlannerSync).omit({ 
  id: true, 
  createdAt: true,
  updatedAt: true,
  localVersion: true,
});
export type InsertHelpdeskPlannerSync = z.infer<typeof insertHelpdeskPlannerSyncSchema>;
export type HelpdeskPlannerSync = typeof helpdeskPlannerSync.$inferSelect;

// Ticket/Feedback comments (for internal communication)
export const helpdeskComments = pgTable("helpdesk_comments", {
  ...
  tenantIdIdx: index("helpdesk_comments_tenant_id_idx").on(table.tenantId),
  ticketIdIdx: index("helpdesk_comments_ticket_id_idx").on(table.ticketId),
  feedbackIdIdx: index("helpdesk_comments_feedback_id_idx").on(table.feedbackId),
}));

export const insertHelpdeskCommentSchema = createInsertSchema(helpdeskComments).omit({ 
  id: true, 
  createdAt: true,
  updatedAt: true,
});
export type InsertHelpdeskComment = z.infer<typeof insertHelpdeskCommentSchema>;
